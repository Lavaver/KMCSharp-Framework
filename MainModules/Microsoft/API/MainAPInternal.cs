...
using KMCSharp.Modules.Microsoft.API;

namespace KMCSharp.MainModules.Microsoft.API
{
    public class MainAPInternal : IMicrosoftAPI
    {
        #region 获取服务器状态
        public Dictionary<string, ServiceStatus> GetServiceStatus()
        {
            ...
            try
            {
                using (WebClient webclient = new WebClient())
                {
                    ...
                    var value = JsonMapper.ToObject(webclient.DownloadString(MainAPIProvider.apiStatus()));
        ...
        public Statistics GetStatistics()
        {
            try
            {
                using (WebClient webclient = new WebClient())
                {
                    ...
                    var value = JsonMapper.ToObject(webclient.UploadString(MainAPIProvider.statistics(), data.ToJson()));
        ...
        public Guid NameToUUID(string userName)
        {
            try
            {
                using (WebClient webclient = new WebClient())
                {
                    var value = JsonMapper.ToObject(webclient.DownloadString(MainAPIProvider.nameToUuid(userName)));
